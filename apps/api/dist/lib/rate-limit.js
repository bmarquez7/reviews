...
const RULES = [
    ...
    },
    {
        ...
        limit: 5,
        windowMs: 30 * 60 * 1000,
        match: (request, pathname) => request.method === 'POST' &&
    ...
    },
    {
        ...
        limit: 40,
        windowMs: 10 * 60 * 1000,
        match: (request, pathname) => ['POST', 'PUT', 'PATCH', 'DELETE'].includes(request.method) &&
...
const pruneHits = (hits, windowMs, now) => hits.filter((hit) => now - hit < windowMs);
export const registerRateLimit = (app) => {
